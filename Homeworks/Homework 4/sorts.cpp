...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    vector<Student*> studentPtr;

    for (int i = 0; i < auxStudents.size(); i++) {
        studentPtr.push_back(&auxStudents[i]);
    }

    sort(studentPtr.begin(), studentPtr.end(), comp);
    
    for (int i = 0; i < studentPtr.size(); i++) {
        students[i] = *studentPtr[i];
    }
